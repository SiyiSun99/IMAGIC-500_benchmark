...
import time, os, json
from remasker_utils_short import NativeScaler, MAEDataset, adjust_learning_rate, get_dataset
import remasker_model_mae
from torch.utils.data import DataLoader, RandomSampler
import sys
import timm.optim as optim_factory
from remasker_utils_short import get_args_parser
...
class ReMasker:
    ...
    def fit(self, X_raw: pd.DataFrame):
        ...
        self.model = remasker_model_mae.MaskedAutoencoder(
            rec_len=dim,
            ...
            for iter, (samples, masks) in enumerate(dataloader):
                ...
                with torch.cuda.amp.autocast():
                    loss, _, _, _ = self.model(
                        samples, masks, mask_ratio=self.mask_ratio
                    )
    ...
    def transform(self, X_raw: torch.Tensor):
        ...
        for i in range(dim):
            imputed_data[:, i] = (
                imputed_data[:, i] * (max_val[i] - min_val[i] + eps) + min_val[i]
            )
