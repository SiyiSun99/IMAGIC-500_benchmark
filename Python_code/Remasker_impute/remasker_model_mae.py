...
class MaskedAutoencoder(nn.Module):
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_1d_sincos_pos_embed(
            self.pos_embed.shape[-1], self.mask_embed.rec_len, cls_token=True
        )
        self.pos_embed.data.copy_(torch.from_numpy(pos_embed).float().unsqueeze(0))

        decoder_pos_embed = get_1d_sincos_pos_embed(
            self.decoder_pos_embed.shape[-1], self.mask_embed.rec_len, cls_token=True
        )
    ...
    def forward(self, data, miss_idx, mask_ratio=0.5):
        ...
        latent, mask, nask, ids_restore = self.forward_encoder(
            data, miss_idx, mask_ratio
        )
