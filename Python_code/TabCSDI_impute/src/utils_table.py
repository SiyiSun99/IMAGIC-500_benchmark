...
def evaluate_ft(model, test_loader, nsample):
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    with torch.no_grad():
        ...
        with tqdm(test_loader, mininterval=5.0, maxinterval=50.0) as it:
            for batch_no, test_batch in enumerate(it, start=1):
                ...
                # Take median of samples for final imputed values
                imputed_values = samples.median(dim=1).values

                # Transform back using tokenizer
                imputed_values = model.tokenizer.recover(
                    imputed_values, len(model.cont_list)
                )

                imputed_data.append(imputed_values.cpu().numpy())
