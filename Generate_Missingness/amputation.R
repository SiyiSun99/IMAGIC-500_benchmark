...
produce_NA <- function(data, 
  ...
  if (mechanism == "MCAR") {
    ...
    if (length(vars_factor) > 0){
      levels_factor <- lapply(vars_factor, function(var) gdata::mapLevels(x=data[,var]))
      names(levels_factor) <- vars_factor
    }
    ...
    if (by.patterns==TRUE) {
      ...
      if (!(mechanism == "MCAR")){
        ...
        if (length(vars_factor) > 0){
          for (i in 1:length(vars_factor)){
            var_name <- vars_factor[i]  
            tmp$data.init[,var_name] <- as.factor(tmp$data.init[,var_name])
            tmp$data.incomp[,var_name] <- as.factor(tmp$data.incomp[,var_name])
            
            gdata::mapLevels(x=tmp$data.init[,var_name]) <- levels_factor[[i]]
            gdata::mapLevels(x=tmp$data.incomp[,var_name]) <- levels_factor[[i]]
          }
        }
